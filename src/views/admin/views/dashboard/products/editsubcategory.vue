<template>
  <v-container id="user-profile" fluid tag="section" class="my-5">
    <v-row justify="center">
      <v-card width="500">
        <v-card-title class="text-h3 white--text primary py-3">
          ویرایش زیر دسته
        </v-card-title>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="gotoback()"> لغو </v-btn>
        </v-card-actions>
      </v-card>
    </v-row>
  </v-container>
    ...
    async updatecategory() {
      ...
      this.$store
        .dispatch("updatecategory", {
          name: this.categgoryname,
          itemId,
        })
        .then(() => {
          alert("اطلاعات زیر دسته به روز رسانی شد");
